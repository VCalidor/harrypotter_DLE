...
import Tips from "../components/emojiChallenge/Tips";
import { Character, emojiCharacter, YesterdayCharacter } from "../interfaces";
...
const EmojiChallenge = () => {
  ...
  const getChosenCharacter = async () => {
    const dailyTries = JSON.parse(localStorage.getItem("emojiTries") || "[]");
    ...
    try {
      const response = await fetch(
        `${API_URL}api/characters/daily-character?type=emoji`,
        {
          method: "GET",
        }
      );
      ...
      const data: {
        tdCharacter: emojiCharacter & { date: string };
  ...
  const getHits = async () => {
    try {
      const response = await fetch(`${API_URL}api/characters/hits?type=emoji`, {
        method: "GET",
      });
  ...
  return (
    <PageLayout isHome={true}>
      <StatsTab mode={"emoji"} hit={hit || alreadyHit} />
      {isTodayCharacter ? (
        <>
          <Tips
            selectedCharactersLength={
              selectedCharacters.length + (lastAddedCharacter ? 1 : 0)
            }
            hit={hit || alreadyHit}
